...
def authenticated_menu():
    ...
    st.sidebar.page_link("pages/news.py", label="건강 정보") # 즐겨찾기 페이지로 이동

    # 관리자 및 슈퍼 관리자 역할에 대한 추가 링크 표시
    if st.session_state.role in ["관리자", "슈퍼 관리자"]:
        #st.sidebar.page_link("pages/admin.py", label="사용자 관리") # 사용자 관리 페이지로 이동
        None
